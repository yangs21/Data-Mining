...
#df = df.drop(12)  #Delete the row with index 12 of df
#df1 = partition[0]
#df2 = partition[1]
    ...
    for i in range(0, k):
        ...
        for m in range(0, size):
            for n in range(0, size):
                x = np.array(partition[i].iloc[m])[0:d_col - 1]
                y = np.array(partition[i].iloc[n])[0:d_col - 1]
    ...
    for j in range(0, n_row):
        for i in range(0, k):
            ...
            for a in range(0, size):
                x = np.array(partition[i].iloc[a])[0:d_col - 1]
                y = D[j, 0:(d_col - 1)]
        ...
        if j in set(list(partition[label].index)):
            ...
            if (j in set(list(partition[i].index)) and i != label):
                partition_t[i] = partition_t[i].drop(j)
